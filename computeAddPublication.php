...
if (isset($_POST['DBLP_Key']) AND isset($_POST['Title']) AND isset($_POST['URL']) AND isset($_POST['EE']) AND isset($_POST['Year']) AND isset($_POST['Crossref']) AND isset($_POST['Note']) AND isset($_POST['type'])){
		...
		if ($_POST['type']=="article"){ $addArticle = $bdd->query('INSERT INTO Article (Publication_id) VALUES ('.$last_insert_id.')');} // Et l'utilise pour créer un article
		if ($_POST['type']=="book"){ $addBook = $bdd->query('INSERT INTO Book (Publication_id) VALUES ('.$last_insert_id.')');} // Ou un livre
		if ($_POST['type']=="masterThesis"){ $addThesis = $bdd->query('INSERT INTO Thesis (Publication_id) VALUES ('.$last_insert_id.')');} // Ou une thèse de master
		if ($_POST['type']=="phdThesis"){ $addThesis = $bdd->query('INSERT INTO Thesis (Publication_id) VALUES ('.$last_insert_id.')'); $addPhdThesis = $bdd->query('INSERT INTO PHDThesis (Publication_id) VALUES ('.$last_insert_id.')');} // Ou une thèse de doctorat		
		$bdd->commit();
